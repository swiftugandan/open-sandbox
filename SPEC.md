...
- **FR-3:** The controller can issue `StartSandbox`, `StopSandbox`, `Exec`, and `FetchLogs` commands to agents over the existing gRPC stream. Agents execute sandbox lifecycle operations via a pluggable container runtime. The default production runtime uses youki/libcontainer (daemonless OCI) for direct in-process container management. A Docker Engine runtime (via Unix socket) is available as a development fallback. Both runtime implementations must ensure the container image is available locally before creating the container — the Docker runtime pulls via Docker Engine API, the youki runtime via oci-client. Image pull is a prerequisite of container creation, not a caller responsibility. The runtime provisions DNS resolution in every container by writing `/etc/resolv.conf` into the rootfs before container start, using the host's nameservers (filtered for localhost entries that are unreachable from the container's network namespace) with a fallback to public DNS (8.8.8.8, 8.8.4.4). On receiving a shutdown signal (SIGTERM/SIGINT), the agent stops and removes all managed containers before exiting.
  ...
  - *Rationale:* Docker Engine writes `/etc/resolv.conf` into containers automatically; daemonless runtimes must handle this explicitly. Filtering 127.0.0.0/8 and ::1 entries is necessary because the host's loopback DNS resolver (e.g. systemd-resolved) is unreachable from the container's own network namespace. Graceful container cleanup on agent shutdown prevents resource leaks and orphaned containers.
...
- **FR-13:** The API gateway supports file operations on running sandboxes: writing files into a sandbox (as a tar.gz upload) and reading files out of a sandbox (as an octet-stream download). File operations are implemented as `ExecCommand` invocations under the hood: write creates the target directory (`mkdir -p`) then extracts an uploaded archive via `tar xzf`, read uses `cat` piped back through the tunnel.
  - *Rationale:* AI agents need to put code into sandboxes and retrieve results. Using exec-backed file operations avoids adding a new data channel to the agent protocol. The tar.gz format handles multiple files, directories, and permissions in a single upload — matching Vercel's sandbox SDK convention [13]. Auto-creating the target directory avoids a separate "mkdir" round-trip from the caller — the common case for AI agents is writing to directories that may not yet exist.
...
- **NFR-API-1:** API error responses include a machine-readable `error_code` string alongside the human-readable `error` message. Error codes are stable identifiers derived from the `ApiError` enum variant names (e.g., `SANDBOX_NOT_FOUND`, `EXEC_FAILED`). Successful mutation responses (file writes) return a structured JSON confirmation body rather than an empty 204. All API error responses use the structured JSON envelope `{"error": "...", "error_code": "..."}`, including framework-level rejections (malformed JSON, missing fields). The API validates request payloads at the boundary — empty command arrays are rejected with `INVALID_REQUEST` before forwarding. File writes without an explicit target directory default to `/home` rather than the filesystem root.
  - *Rationale:* Programmatic API consumers (AI agents) need stable identifiers for error handling. Free-text error messages are for humans; error codes are for code. Structured success responses provide confirmation that the operation completed. Consistent error envelopes across all failure modes prevent SDK consumers from having to handle two different error formats. Boundary validation catches invalid input early, before it reaches downstream services.
