...
- **Daily iteration** (any host, including macOS): `Dockerfile.dev` + `docker-compose.dev.yml`. Long-lived dev container with bind-mounted source and named volumes for `target/` + cargo registry. `docker compose -f crates/agent-youki/docker-compose.dev.yml up -d` once, then `docker compose -f ... exec dev cargo test -p open-sandbox-agent-youki -- --nocapture`. Cargo's incremental compile makes typical edits ~seconds. For startup-time benchmarking: `cargo run --release --example bench_create_and_start -p open-sandbox-agent-youki` inside the same container (the example file's module doc explains the methodology + how to compare against the docker-runtime numbers from `measure.py`).
