...
- **NFR-OBS-1:** The controller exposes Prometheus metrics: connected agents, active sandboxes, heartbeat latency histogram, sandbox start/stop rates, routing-table size. Agent components emit structured tracing events at key lifecycle points: connection, registration, sandbox start/stop, exec dispatch, image pull.
