...
**Ready to merge:** `contracts/v1.0.3` on branch `contracts/amendment-live-edit-v1.0.3` (2026-05-28, ~30 commits ahead of `main`). The "Replit in a tab" feature — full live-edit batch covering all four groups of `docs/plans/PLAN_LIVE_EDIT_TASKS.md`:
- **Group A — contracts (4 commits):** `ListDirParams` / `WaitPortListeningParams` / `FileMeta` sidecar / write `expected_revision` precondition. `IoErrorCode` gains `RevisionMismatch` + `NotImplemented` variants; `ApiError` gains matching variants. Frame envelope kinds centralized in `crates/contracts/src/constants.rs` (closes the dual-table drift risk).
- **Group B — agent runtime (8 commits):** `ContainerRuntime::list_dir` + `stat_revision` trait methods. `drive_list_dir` + `drive_wait_port_listening` handlers in `crates/agent/src/io_stream.rs`. `HostPortLookup` trait so `drive_io_session` can resolve sandbox host ports without coupling to the full `SandboxManager`. `drive_read_file` emits `FileMeta` before the first Stdout chunk; `drive_write_file` enforces `expected_revision` via the new `runtime.stat_revision` call and emits `FileMeta` on the post-write ACK. Real docker (`ls -lAn --time-style=+%s` exec parse) and youki (`setns(2)` + `std::fs::read_dir` + `std::fs::metadata` follows-symlinks) impls.
- **Group C — API gateway (2 commits):** `GET /v1/sandboxes/{id}/files/list?path=&cwd=` + `POST /v1/sandboxes/{id}/wait_port_listening`. `X-File-Revision` response header on `read_file` (in CORS `expose_headers`). `WriteFileRequest` gains optional `expected_revision` + `force`. `REVISION_MISMATCH` → HTTP 409 with `{actual_revision}` JSON body; `NOT_IMPLEMENTED` → 501. `FileMeta` flows over the WS streaming read endpoint as a JSON Text frame before the first Binary chunk.
- **Group D — UI (15 commits):** new `Edit` tab in the Next.js console hosting a three-column layout — lazy `FileTree` (`api.listDir`, default-hidden directories, `Cmd-Shift-H` toggle) + tabbed CodeMirror 6 editor (lazy lang packs, document-level `Cmd-S` with input-context gate, 5s blur-autosave with in-flight guard, IndexedDB unsaved-buffer persistence per `{sandboxId, path}`, 409 conflict banner with Reload/Overwrite, 30s on-visibility revision re-check) + preview iframe with debounced (200ms) save-chain reload (`writeFile` → `waitPortListening` → cache-bust). Mobile (<768px) drops tree + editor and renders preview-only with a "view on desktop" hint.

Nine `/code-review` passes during development; every high-severity finding (cross-sandbox state bleed, dual `@codemirror/view` import risk, blur-autosave double-save race, drain_remaining_client_frames coverage gaps, etc.) closed in the same iteration it was found. 21 medium/low-severity items tracked in `docs/reviews/FOLLOWUPS_v1.0.3.md`.

**Tag status:** `contracts/v1.0.2` tag still points at the initial v1.0.2 commit (`0e68177`, predates #13); tag movement deferred until items #1–#12 ship. `contracts/v1.0.3` + `contracts/v1.0.3-frozen` + `module/live-edit/done` tag movement waits for the live-edit branch to merge to `main` AND a live-environment smoke (open dev console, create sandbox, edit file in the Edit tab, observe save → wait_port_listening → preview iframe refresh).

**Pending work, by category:**

- **v1.0.2 amendments #1–#12** (`docs/plans/PLAN_CONTRACTS_v1.0.2.md`): `IoErrorCode` enum (partially closed by v1.0.3's RevisionMismatch + NotImplemented additions), structured error trailer, `Port`/`Signum`/`SUBDOMAIN_LEN` newtypes, etc. Separate completion session.
- **v1.0.3 follow-ups** (`docs/reviews/FOLLOWUPS_v1.0.3.md` items #16–#22, #33–#36): busybox-compatible `list_dir` fallback (GNU `ls --time-style=+%s` doesn't work on Alpine), `Sandbox.exposed_port` plumbed through SandboxInfo so the save-chain probes the right port, dedicated `/files/stat` endpoint so D12's 30s re-check doesn't pull the full file body, IndexedDB write debouncing.
