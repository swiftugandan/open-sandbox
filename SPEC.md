...
- **FR-3:** The controller can issue `StartSandbox`, `StopSandbox`, `Exec`, and `FetchLogs` commands to agents over the existing gRPC stream. Agents execute sandbox lifecycle operations via a pluggable container runtime. The default production runtime uses youki/libcontainer (daemonless OCI) for direct in-process container management. A Docker Engine runtime (via Unix socket) is available as a development fallback. Both runtime implementations must ensure the container image is available locally before creating the container — the Docker runtime pulls via Docker Engine API, the youki runtime via oci-client. Image pull is a prerequisite of container creation, not a caller responsibility. v1.0.2 adds a caller-supplied `pull_policy` field on `CreateSandboxRequest` (kebab-case JSON: `"if-not-present"` (default), `"always"`, `"never"`) that the runtime honors: `IfNotPresent` skips the registry round-trip when the image is locally cached (matches `docker run` semantics and saves ~1.1s p50 on warm starts measured on the dev fleet 2026-05-26); `Always` restores the v1.0.1 always-pull behavior for floating tags like `:latest`; `Never` fails fast if the image is absent locally (air-gapped / strict-pin deployments). The agent emits an `image present locally; skipping pull` info event on the warm path in place of the `pulling image` / `image pull complete` pair. Both runtimes override the container entrypoint to `["sh", "-c", "mkdir -p /workspace 2>/dev/null; exec sleep infinity"]`, keeping the sandbox idle for exec-based interaction — YoukiRuntime sets OCI process args, DockerRuntime sets the container `Cmd` field. The `mkdir -p /workspace` seeds the directory the Edit-tab file tree opens to so it shows an existing (possibly empty) directory before any user-supplied process runs; `2>/dev/null;` (sequence, not `&&`) fails open on read-only or non-root-writable `/` so a hardened image still reaches sleep. `exec` replaces the shell with sleep so SIGTERM to PID 1 propagates as expected. Distroless / scratch images without `/bin/sh` will fail to start; per-sandbox entrypoint override is a deferred config knob. The runtime provisions DNS resolution in every container by writing `/etc/resolv.conf` into the rootfs before container start, using the host's nameservers (filtered for localhost entries that are unreachable from the container's network namespace) with a fallback to public DNS (8.8.8.8, 8.8.4.4). On receiving a shutdown signal, the agent handles both SIGINT and SIGTERM for graceful shutdown, ensuring container cleanup in both interactive and Docker/systemd environments.
